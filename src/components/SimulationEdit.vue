  ...
  <v-dialog v-model="dialog" width="500px" :fullscreen="$vuetify.breakpoint.xsOnly" persistent>
    <v-card :tile="$vuetify.breakpoint.xsOnly" class="d-flex flex-column">
      <v-card-title class="warning white--text">
        <span class="headline">Ajustar Dados Básicos</span>
        <v-spacer />
...
export default {
  ...
  methods: {
    open (simulation) {
      ...
      this.rollback.name = this.simulation.name
      this.rollback.area = this.simulation.area
      this.rollback.farm = this.simulation.farm
    ...
    cancel () {
      ...
      this.simulation.name = this.rollback.name
      this.simulation.area = this.rollback.area
      this.simulation.farm = this.rollback.farm
    ...
    saveSimulation () {
      ...
      this.$db.simulation
        ...
        .modify(this.simulation).then(() => {
          this.$emit('message', 'Simulação ajustada com sucesso!')
